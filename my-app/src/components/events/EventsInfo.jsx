import React from 'react';
import { useState, useRef } from 'react';
import { useHistory } from 'react-router-dom';
import Button from '@material-ui/core/Button';
import { Link } from 'react-router-dom';
import { useAuth } from '../context/AuthContext'
import './EventsInfo.css'

export const EventInfo = () => {
    const emailRef = useRef();
    const passwordRef = useRef();
    const [error, setError] = useState('');
    const [loading, setLoading] = useState(false);

    let history = useHistory();

    return (
        <div className="px-4 p-sm-0 customBg">

            {/* Dashboard */}
           

            <div className="maincontainer">
                <div className="container-fluid">
                    <div className="row innerrow bg-white shadow rounded overflow-hidden">
                    {/* <div className=""> */}
                    <div id="carouselExampleControls" class="carousel slide" data-bs-ride="carousel">
                                <div class="carousel-inner">
                                    <div class="carousel-item active">
                                    <img className="d-block w-100 card-img-top" src="https://therichpost.com/wp-content/uploads/2021/02/login-split.jpg" alt="Card image cap" />
                                    </div>
                                    <div class="carousel-item">
                                    <img className="d-block w-100 card-img-top" src="data:image/jpeg;base64," alt="Card image cap" />
                                    </div>
                                    <div class="carousel-item">
                                    <img className="d-block w-100 card-img-top" src="https://therichpost.com/wp-content/uploads/2021/02/login-split.jpg" alt="Card image cap" />
                                    </div>
                                </div>
                                <button class="carousel-control-prev" type="button" data-bs-target="#carouselExampleControls" data-bs-slide="prev">
                                    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                    <span class="visually-hidden">Previous</span>
                                </button>
                                <button class="carousel-control-next" type="button" data-bs-target="#carouselExampleControls" data-bs-slide="next">
                                    <span class="carousel-control-next-icon" aria-hidden="true"></span>
                                    <span class="visually-hidden">Next</span>
                                </button>
                            </div>
                    
                    <div class="bg-light p-4 d-flex justify-content-center text-center">
                        <ul class="list-inline mb-0">
                            <li class="list-inline-item">
                                <h5 class="font-weight-bold mb-0 d-block mr-b">Event Title</h5><small class="text-muted"> <i class="fas fa-calendar-week"></i> Time  </small><small class="text-muted mr-l"> <i class="fas fa-map-marker-alt"></i> Place </small>
                            </li>
                            
                            
                        </ul>
                    </div>
                    <div className="px-4 py-3">
                        <div className="d-flex align-items-center justify-content-between mb-3 heads">
                            <h5 className="mb-0 ">Event Information</h5>
                        </div>
                        <div className="p-4 rounded shadow-sm bg-light">
                            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi ac gravida odio, id blandit ipsum. Donec sit amet bibendum tortor, a dignissim metus. In hac habitasse platea dictumst. Donec mollis ut odio eu suscipit. In a sapien neque. Nam congue sapien et eros eleifend, vel rutrum arcu eleifend. Vivamus fringilla volutpat ultricies.

Integer condimentum porttitor sapien ullamcorper fringilla. Donec ultrices purus vel tempus facilisis. Cras pulvinar est et lectus dignissim, ac congue arcu suscipit. Nunc nec nulla semper, sollicitudin neque eget, laoreet ligula. Mauris non tempor arcu. Proin ut posuere ante, sit amet eleifend magna. Aliquam erat volutpat. Mauris facilisis, nunc eu vestibulum rutrum, lorem lacus pulvinar diam, id convallis erat erat ut eros. Vivamus diam nulla, cursus eu urna in, mollis ultricies risus. Fusce tempus condimentum velit, quis ullamcorper erat hendrerit in. Sed elit dui, semper non arcu vel, interdum sagittis est. Maecenas eu velit quis sem facilisis efficitur suscipit eu mi. Etiam tincidunt congue magna, et pulvinar erat lobortis vitae. Cras eget pulvinar tellus. Phasellus iaculis vel tellus ac vulputate. Proin porttitor eget odio eget vestibulum.

In pellentesque, urna fringilla cursus tempor, ligula nisl rutrum magna, non tempor mauris dolor id nisl. Maecenas vel nisi rutrum, rhoncus augue sed, efficitur risus. Nullam pretium molestie semper. Aenean hendrerit egestas nisi, vitae pellentesque arcu venenatis eget. Aenean id arcu mollis, vestibulum arcu non, efficitur mauris. Aenean non venenatis magna, in elementum nulla. Quisque sit amet sollicitudin odio. Phasellus est sem, congue sit amet dignissim a, condimentum ut arcu. Nunc hendrerit massa a mi fringilla bibendum ac vitae libero.

Suspendisse et elit sodales, aliquam enim sed, egestas sapien. Aliquam erat volutpat. Curabitur eget augue mattis turpis pharetra tristique. Proin vulputate sapien id augue fermentum, ut cursus sem vulputate. Nulla facilisi. Nulla scelerisque neque arcu, quis efficitur dolor volutpat sed. Donec luctus vitae sapien vel malesuada. Sed accumsan nec eros pharetra convallis. Nam eu neque sed felis vehicula dapibus. Mauris ornare euismod odio, in pretium purus condimentum a. Sed efficitur quis eros quis dapibus. Proin dapibus quam at pharetra sollicitudin. Suspendisse arcu lorem, consectetur ut ultrices sed, porttitor eu lectus. Donec bibendum fringilla magna sit amet congue.</p>
                        </div>
                    </div>
                    <Button variant="contained" class = "btn">Register</Button>
                    
                    </div>
                </div>
            </div>
        </div>
    );
};
